...
export function StockGains({ stockName }: StockGainsProps) {
  ...
  const { data, isLoading, error } = useFetchStockGains(
    stockName,
    date,
    amount
  );
  ...
  const dateIsoText = useMemo(
    () => (date ? Formatter.isoText(date) : ""),
    [date]
  );
    ...
    <Box data-testid="stock-gains">
      ...
      {!isLoading && data && (
        <Box display="flex" flexDirection="column" alignItems="center">
          ...
          <Typography
            data-testid="gains-label"
            variant="h2"
            color={data.capitalGains > 0 ? "limegreen" : "red"}
          >
            {signal} {Formatter.brlCurrency(Math.abs(data.capitalGains))}
          </Typography>
          <Box>
            <Typography
              data-testid="gains-old-price-label"
              color="gray"
              align="center"
            >
              Price at{" "}
              {Formatter.dateUTC(new Date(data.purchasedAt), "dd/MM/yyyy")}:{" "}
              {Formatter.brlCurrency(data.priceAtDate)}
            </Typography>
            <Typography
              data-testid="gains-current-price-label"
              color="gray"
              align="center"
            >
